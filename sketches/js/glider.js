...
const seedPath = '../../seeds/glider.json';

// set resolution to a divisor of width and height
const [ width, height ] = [ 540, 540 ];
const resolution = 30;
...
new p5(function(p5) {
  ...
  p5.draw = function() {
    ...
    drawGame();
    game = game.evolve();
    if (game.origin.x === 20) game = new Game(seed.grid);
  };
  ...
  function drawGame() {
    ...
    for (let y in game.cells) {
      ...
      for (let x in game.cells[y]) {
        ...
        const cellX = (x - 2) * resolution
        const cellY = (y - 2) * resolution 
        if (game.cells[y][x]) p5.square(cellX, cellY, resolution, 8);
      }
    }
  }
});
